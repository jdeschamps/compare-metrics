...
public class TRA
{
	 ...
	 */
	public void CheckConsistency(final Vector< TrackDataCache.TemporalLevel > levels,
			final Map<Integer, TrackDataCache.Track > tracks,
			final boolean isGTcheck)
	{
		...
		for ( TrackDataCache.Track track : tracks.values())
		{
		...
		for (int t = 0; t < levels.size(); ++t)
		{
			...
			for (int id : idArray)
			{
				...
				final TrackDataCache.Track track = tracks.get(id);
	 ...
	 */
	protected int GetGTMatch(final TrackDataCache.TemporalLevel level, final int lbl)
	{
	 ...
	 */
	protected Collection<Integer> GetResMatch(final TrackDataCache.TemporalLevel level, final int lbl)
	{
	 ...
	 */
	protected boolean ExistGTEdge(final Vector< TrackDataCache.TemporalLevel > levels,
			...
			boolean[] parental) //an output variable...
	{
		...
		if (start_index != -1 && end_index != -1)
		{
			...
			else
			{
				...
				final TrackDataCache.Track parent = tracks.get(start_label);
				final TrackDataCache.Track child = tracks.get(end_label);
	 ...
	 */
	protected boolean ExistResEdge(final Vector< TrackDataCache.TemporalLevel > levels,
			final int start_level,
			final int start_index,
			final int end_level,
			final int end_index,
	...
	protected void FindEDAndECEdges(final Vector< TrackDataCache.TemporalLevel > levels,
			final Map<Integer, TrackDataCache.Track > gt_tracks,
			final Map<Integer, TrackDataCache.Track > res_tracks)
	{
		...
		for (Integer res_track_id : res_tracks.keySet())
		{
			...
			final TrackDataCache.Track res_track = res_tracks.get(res_track_id);
	...
	protected void FindEAEdges(final Vector< TrackDataCache.TemporalLevel > levels,
			final Map<Integer, TrackDataCache.Track > gt_tracks,
			final Map<Integer, TrackDataCache.Track > res_tracks)
	{
		int start_level, end_level;
		...
		{
			//short-cut to the track data
			final TrackDataCache.Track gt_track = gt_tracks.get(gt_track_id);
			...
			throws IOException, ImgIOException
	{
		...
		final HashMap<Integer, TrackDataCache.Track > gt_tracks  = cache.gt_tracks;
		final HashMap<Integer, TrackDataCache.Track > res_tracks = cache.res_tracks;
		final Vector< TrackDataCache.TemporalLevel > levels = cache.levels;
		...
		for ( TrackDataCache.TemporalLevel level : levels)
		{
	...
	//	if ((max_split - 1) * penalty.m_ns > (penalty.m_fp + max_split * penalty.m_fn))
		...
		if (doAOGM == false)
		{
			...
			{
				TrackDataCache.Track t = gt_tracks.get(id);
